...
(اكتب الكلمات هنا)
  ...
  const LoadingIndicator = ({ task }: { task: string }) => (
    <div className="flex flex-col items-center justify-center p-8 space-y-4">
      ...
      <p className="text-indigo-300 font-bold animate-pulse">{task}...</p>
    </div>
  );
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-[#09090b] text-slate-200' : 'bg-slate-50 text-slate-900'} font-sans p-3 md:p-6 transition-colors duration-500`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {(outputResult || isLoading) && (
          <div className={`${darkMode ? 'bg-[#121217] border-slate-800 shadow-indigo-500/10' : 'bg-white border-slate-200 shadow-slate-200'} rounded-3xl border shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-500 transition-all`}>
